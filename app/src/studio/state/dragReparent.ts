 ...
 * 판정 기준은 드래그된 노드의 **중심점**이 그룹 프레임 rect 안/밖인지다 (절대좌표 기준,
...
function attach(
  ...
  return [...nodes.filter((node) => node.id !== dragged.id), next];
}

/**
 * 드래그 종료된 노드의 그룹 탈착/부착을 판정해 새 노드 배열을 반환한다. 변화 없으면 null.
 * - 그룹 자식: 중심점이 부모 프레임 밖이면 detach (다른 펼쳐진 그룹 위면 그 그룹으로 재부착).
 * - 독립 sop_task: 중심점이 펼쳐진 그룹 프레임 안이면 해당 그룹에 attach.
 */
export function resolveDragReparent(
  ...
  if (parent) {
    if (containsPoint(parent, center)) {
  ...
  const group = findExpandedGroupAt(nodes, center, new Set([draggedId]));
