...
## Current goal
Phase 6 — 3단계: 실내공간정보 구축 작업규정 분석 → 레이어 분류/명명규칙 반영 공간 스키마 설계·적용, 검증용 3D 공간 모델링, 1단계 Space 노드 연동 보완 (`/phase-run 6`으로 시작)
...
## Next steps
1. `/phase-run 6` — 실내공간정보 구축 작업규정(별표 2·3·5·6·10) 분석 → 공간 스키마 설계·적용 + 검증용 3D 공간 모델 + Space 노드 연동 보완 (3단계)
2. Phase 7 — webbuilder 토폴로지 연동 (4단계)
